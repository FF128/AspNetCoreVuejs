            ...
            <v-data-table
             ...
             <look-up-dialog :dialog="transDialog" :title="'Budget Details Header'" v-if="transDialog">
                <template slot="table">
                    <v-data-table
                        ...
                        <template slot="items" slot-scope="props">
                            ...
                            <td>
                                <v-btn icon
                                    @click.prevent="selectBudgetEntry(props.item)">
                ...
                <template slot="card-actions">
                    <v-btn
                        flat="flat"
                        @click.prevent="transDialog = false">
                        Close
                    </v-btn>
                </template>
            </look-up-dialog>
...
export default {
    ...
    data() {
        return {
            ...
            budgetEntry: this.initializeBudgetEntry(),
            allItems: [],
            items: [],
            bedHeaders: [
                {
    ...
    methods: {
        ...
        getAllItemsPagination() {
            this.tableLoading = true
            this.$axios.get(`${this.apiEndpoint}/details/pageNum/${this.pagination.page}/pageSize/${this.pagination.rowsPerPage}/query/${this.query}`)
                .then(({ data }) => {
                    this.allItems = data;
        ...
        getItemsByTransNo() {
            ...
            this.$axios.get(`${this.apiEndpoint}/details/${this.budgetEntry.transactionNo}`)
                .then(({ data }) => {
                    this.items = data;
                    this.tableLoading = false
                })
        ...
        getMainHeader() {
            this.$axios.get(`${this.apiEndpoint}/main`)
                .then(({ data }) => {
        ...
        selectBudgetEntry(item) {
            this.budgetEntry = item;
