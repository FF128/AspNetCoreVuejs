<template>
  <div>
    <v-container>
      <h1>{{title}}</h1>
      <!-- <v-layout row wrap> -->
      <v-data-table :headers="headers" :items="prEntryData">
        <template slot="items" slot-scope="props">
          <td>
            <v-btn flat icon @click.prevent="$router.push(`pr-approval/${props.item.prfNo}`)">
...
export default {
  ...
  methods: {
    ...mapActions("prEntry", ["getAllPREntryData"])
  },
  computed: {
    ...mapState("prEntry", {
