<template>
  <div>
    <v-progress-linear :indeterminate="true" class="ma-0" :active="pageLoading" v-if="pageLoading"></v-progress-linear>
    <v-form>
      <v-container>
        <v-layout>
        ...
        <v-layout row wrap>
          <v-flex xs12 sm6 md4>
            <v-text-field v-model="prMain.prfNo" label="PRF No." readonly></v-text-field>
          </v-flex>
          <v-flex xs12 sm6 md4>
            <v-text-field
          ...
          <v-flex xs12 sm6 md4>
            <v-text-field v-model="prMain.durationFrom" label="Duration From" type="date" readonly></v-text-field>
          </v-flex>
          <v-flex xs12 sm6 md4>
            <v-text-field v-model="prMain.durationTo" label="Duration To" type="date" readonly></v-text-field>
          </v-flex>
          <v-flex xs12 sm6 md4>
            <v-text-field v-model="prMain.dateRequired" label="Date Required" type="date" readonly></v-text-field>
          </v-flex>
          <v-flex xs12 sm6 md4>
            <v-text-field v-model="prMain.requestedBy" label="Requested By" readonly></v-text-field>
          </v-flex>
          <v-flex xs12 sm6 md4>
            <v-text-field v-model="prMain.empStatus" label="Employment Status" readonly></v-text-field>
          </v-flex>
          <v-flex xs12 sm6 md4>
            <v-text-field v-model="prMain.reason" label="Reason for Personnel Request" readonly></v-text-field>
          </v-flex>
          <v-flex xs12 sm6 md4>
            <v-text-field v-model="prMain.remarks" label="Remarks" readonly></v-text-field>
          </v-flex>
        </v-layout>
...
export default {
  ...
  methods: {
    ...
    accept() {
      this.$axios
        .post(`${this.apiEndpoint}/accept`, this.prMain)
        .then(({ data }) => {
          toast.show(data);
        })
        .catch(({ response }) => {
    ...
    decline() {
      this.$axios
        .post(`${this.apiEndpoint}/decline`, this.prMain)
        .then(({ data }) => {
          toast.show(data);
        })
        .catch(({ response }) => {
